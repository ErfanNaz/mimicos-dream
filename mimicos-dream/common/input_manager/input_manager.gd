...
func is_action_pressed(action: int) -> bool:
	return controller_input.buttons.get(action, 0) > 0
	
func is_action_holded(action: int) -> bool:
	return controller_input.buttons.get(action, 0) >= 0.6
	

func _process(_delta: float) -> void:
	if has_input_process:
		input_process.emit(_delta)
		return
	if !active:
		return
	...
	if controller_input.direction != direction:
		#print_debug("direction changed version %s direction from %s to %s " % [controller_input.version, controller_input.direction, direction])
	...
	for button_action in button_actions:
		var prea_state: float = controller_input.buttons.get(index, 0)
		var action_triggered: bool = Input.is_action_pressed(button_action)
		if action_triggered:
			if prea_state == 0:
				#print_debug("button changed version %s button from %s to %s " % [controller_input.version, prea_state, 1])
				buttons_diff.set(index, 1)
				has_button_changed = true
		if prea_state > 0:
			#print_debug("button changed prea_state > 0 version %s button from %s" % [controller_input.version, prea_state])
			if !action_triggered:
				#print_debug("button changed version %s button from %s to %s " % [controller_input.version, prea_state, 0])
				buttons_diff.set(index, 0)
				has_button_changed = true
			else:
				if prea_state == 1:
					#print_debug("button changed version %s button from %s to %s " % [controller_input.version, prea_state, 0.7])
					buttons_diff.set(index, 0.7)
					has_button_changed = true
		index = index + 1
	...
	if enable_forces:
		...
		if force_right != controller_input.force_right:
			...
			has_changed = true
	if Input.is_action_pressed(menu_button_actions[1]):
